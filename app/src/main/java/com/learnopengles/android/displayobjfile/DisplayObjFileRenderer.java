...
import com.learnopengles.android.objects.ObjFile;
...
public class DisplayObjFileRenderer implements GLSurfaceView.Renderer
{
	...
	public DisplayObjFileRenderer(final DisplayObjFileActivity displayObjFileActivity, final GLSurfaceView glSurfaceView)
	{
        ...
        mObjFile = new ObjFile(mDisplayObjFileActivity);
	}
	...
	public void onSurfaceCreated(GL10 glUnused, EGLConfig config) 
	{
        ...
        float color_bright_white[] = new float[] { 0.8f, 0.8f, 0.8f, 1.0f };
        ...
        mObjFile.build_buffers(color_bright_white);
...
//        drawCylinder();
        ...
        float maxX = mObjFile.mMaxX;
        float maxY = mObjFile.mMaxY;
        float maxZ = mObjFile.mMaxZ;
        float aveMax = (maxX + maxY + maxZ)/2.0f;
        float scaleF = 1.0f / aveMax;
        // Obj #2 center
        Matrix.setIdentityM(mModelMatrix, 0);
...
//        drawCone();
    ...
    /* asset obj */
    private void drawAssetObj()
    {
        // Pass in the position information
        mObjFile.render(mPositionHandle,
                mColorHandle,
                mNormalHandle,
                mWireFrameRenderingFlag
        );
    ...
    public void toggleShader() {
        if (mUseVertexShaderProgram) {
            ...
            mDisplayObjFileActivity.updateShaderStatus(false);
        } else {
            mUseVertexShaderProgram = true;
            mDisplayObjFileActivity.updateShaderStatus(true);
        }
    }
    ...
    public void toggleWireframeFlag() {
        if (mWireFrameRenderingFlag) {
            ...
            mDisplayObjFileActivity.updateWireframeStatus(false);
        } else {
            mWireFrameRenderingFlag = true;
            mDisplayObjFileActivity.updateWireframeStatus(true);
        }
    }
    ...
    public void toggleRenderIBOFlag() {
        if (mRenderOnlyIBO) {
            ...
            mDisplayObjFileActivity.updateRenderOnlyIBOStatus(false);
        } else {
            mRenderOnlyIBO = true;
            mDisplayObjFileActivity.updateRenderOnlyIBOStatus(true);
        }
    }

    public void loadObjFile() {
        mObjFile.parse(mObjFileName);
