...
public class LessonSevenActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.button_switch_shaders).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				toggleStride();
			}
	...
	public void updateStrideStatus(final boolean useStride) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				if (useStride) {
					((Button) findViewById(R.id.button_switch_shaders)).setText(R.string.lesson_seven_using_stride);
				} else {
					((Button) findViewById(R.id.button_switch_shaders)).setText(R.string.lesson_seven_not_using_stride);
				}
			}
		});
	}
